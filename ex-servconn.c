...
int SCconnect(char *host) {
	char buf[512];

	/* connect to server */
	if ((pop_fd = connect_to_server(host, POP_PORT)) == -1) return -1;

	/* buffer reading part */
	if ((infp = fdopen(pop_fd, "r")) == NULL) return -1;
 ...
 */
{
	...
	if (pop_fd == -1) return -1;
	if (len + 3 > MAXMSG) return -1;
	sprintf(buf, "%s\r\n", str);
	if (write(pop_fd, buf, len + 2) != len + 2) return -1;
	return 0;
}
